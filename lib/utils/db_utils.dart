...
class DatabaseUtils {
  ...
  DatabaseUtils._internal() {
    getDatabasesPath().then((path) async {
      _db = await openDatabase(join(path, _dbName), version: 1,
          onCreate: (db, version) {
...
//      ProvinceModel.fromProvinceTableList(await _db
//          .rawQuery('select province_id, province_name from $_tableProvinces'));
//
//  Future<List<ProvinceModel>> queryAllCitiesInProvince(String proid) async =>
//      ProvinceModel.fromCityTableList(await _db.rawQuery(
//        'select city_id, city_name from $_tableCities where province_id = ?',
//        [proid],
//      ));
//
//  Future<List<DistrictModel>> queryAllDistrictsInCity(String cityid) async =>
//      DistrictModel.fromDistrictTableList(await _db.rawQuery(
...
//  }
//
//  Future<void> insertCitiesInProvince(
//      List<ProvinceModel> cities, String proid) async {
//    var batch = _db.batch();
...
//  }
//
//  Future<void> insertDistrictsInCity(
//      List<DistrictModel> districts, String cityid) async {
//    var batch = _db.batch();
